...
router.post('/voice/sessions/:id/request-staff',(req,res)=>{try{const session=getVoiceSession(req.params.id);const staff={id:`STAFF-${randomUUID()}`,status:'NEEDS_STAFF',transcript:session.rawTranscript,claimedBy:null,updatedAt:new Date().toISOString()};staffSessions.set(staff.id,staff);session.status='STAFF_ASSIST';session.staffSessionId=staff.id;session.updatedAt=new Date().toISOString();emit('voice.staff_requested',{sessionId:session.id,staffSession:staff});emit('staff.session_updated',staff);res.status(201).json({session,staffSession:staff});}catch(e:any){res.status(400).json({error:e.message});}});
