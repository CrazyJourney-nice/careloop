...
const OUT = "/Users/cj/careloop/CMICHK/Careloop_MICHK_2026_Final.pptx";
const RENDER = "/Users/cj/careloop/CMICHK/build/rendered";
const ASSET = "/Users/cj/careloop/CMICHK/build/assets";

const W = 1280, H = 720;
const C = {
  bg: "#FBF7F0", paper: "#FFFDFC", navy: "#17345C", navy2: "#294D73",
  teal: "#087F79", teal2: "#58B7A8", tealPale: "#DFF2EC",
  coral: "#EC5A5F", coralPale: "#FBE4E2", sand: "#EFE4D4",
  ink: "#17233A", muted: "#667085", white: "#FFFFFF", line: "#D8D2C8",
  amber: "#D99B2B", green: "#2C8C67", gray: "#A8B0BA"
};
...
// 1
{
  ...
  addImage(s, `${ASSET}/ui_hero.png`, 670, 70, 540, 560, { alt: "Careloop current dining UI evidence", fit: "cover" });
  ...
  notes(s, "我们不是再开一家食堂，而是在做一个可以嵌入社区、把一顿饭变成持续照护触点的服务节点。当前可展示的是软件与交互原型；硬件部署和商业收入属于下一阶段。", ["Careloop prior deck, Group 5-暖桌Careloop.pdf, slide 14 (user-provided UI evidence)"]);
}
...
// 6
{
  ...
  addImage(s,`${ASSET}/community_concept.png`,62,188,450,430,{alt:"Community care station concept render from prior Careloop deck",fit:"cover"});
  const layers=[
  ...
  notes(s,"这页第一次把系统讲完整。AICAN等设备提供底层自动化；Careloop增加订单状态、四角色界面、人工协作和运营数据。左侧是概念空间图，不代表已部署站点。", ["AICAN Global official overview, 2026, https://www.aicanglobal.com/", "Careloop prior deck, Group 5-暖桌Careloop.pdf, slide 16 (concept render)"]);
}
...
// 10
{
  ...
  addImage(s,`${ASSET}/ui_elder.png`,62,190,355,420,{alt:"Careloop elder-side current UI crop",fit:"cover"});
  const roles=[
    ["老人端","自己点","语音 / 大按钮 / 预览确认",C.coral],
    ["家属端","帮他点","远程代点 / 套餐 / 状态",C.teal],
    ["工作人员端","接住异常","订单队列 / 偏好 / 协作",C.coral],
    ["运营端","产生数据","趋势 / 设备 / 人工介入",C.teal]
  ];
  roles.forEach((d,i)=>{const x=462+(i%2)*375,y=190+Math.floor(i/2)*210; box(s,x,y,340,180,d[0],`${d[1]}\n${d[2]}`,d[3],{bodySize:21});});
  notes(s,"四个终端不是四个独立App，而是同一笔订单的四个角色视图。老人看到简单动作，家属看到远程协作，工作人员看到异常，运营端看到改进数据。", ["Careloop prior deck, Group 5-暖桌Careloop.pdf, slide 14 (user-provided UI evidence)"]);
}

// 11
{
  const s=base(11,"真正体现 Care 的，是 AI 不确定时有人接住","DEMO / VALIDATED / ASSUMPTION");
  addImage(s,`${ASSET}/ui_workflow.png`,62,196,420,390,{alt:"Careloop current workflow UI crop",fit:"cover"});
  addText(s,"NORMAL PATH",535,200,260,26,16,C.teal,{bold:true});
  const normal=["老人发起","生成预览","确认订单","厨房 / 人员接单","完成并记录"];
  normal.forEach((t,i)=>{const x=535+i*133; addShape(s,"roundRect",x,245,112,70,C.tealPale,{line:C.teal,lineWidth:1,radius:"rounded-xl"}); addText(s,t,x+8,265,96,34,17,C.teal,{bold:true,align:"center",valign:"middle"}); if(i<4)arrowText(s,x+110,260,25,C.teal);});
  addText(s,"UNCERTAIN PATH",535,365,270,26,16,C.coral,{bold:true});
  const fail=["识别不确定","停止自动推进","进入任务队列","工作人员确认","继续 / 改单"];
  fail.forEach((t,i)=>{const x=535+i*133; addShape(s,"roundRect",x,410,112,70,C.coralPale,{line:C.coral,lineWidth:1,radius:"rounded-xl"}); addText(s,t,x+8,430,96,34,17,C.coral,{bold:true,align:"center",valign:"middle"}); if(i<4)arrowText(s,x+110,425,25,C.coral);});
  addText(s,"现场演示：视频 + 6张截图备份；真实机器人 / 支付 / 传送带不得冒充已接入",535,535,665,54,19,C.ink,{bold:true,align:"center"});
  notes(s,"演示必须同时跑一次正常路径和一次失败转人工。正常路径证明闭环，失败路径证明责任边界。现场建议准备90秒视频和六张连续截图；支付、AICAN和传送带只作为Demo adapter。", ["Careloop current UI and demo evidence referenced in user-provided v3 guide"]);
}

// 12
{
  const s=base(12,"订单数据先形成 Operational Learning，再谈壁垒","ROADMAP / TO VALIDATE");
  addShape(s,"roundRect",62,210,360,340,C.navy,{radius:"rounded-xl"});
  addText(s,"一张订单留下什么？",92,240,300,38,27,C.white,{bold:true});
  ["偏好与菜单选择","人工介入与异常类型","设备利用与菜品耗时","高峰与服务覆盖"].forEach((t,i)=>{pill(s,t,92,310+i*55,300,i===1?C.coralPale:C.tealPale,i===1?C.coral:C.teal);});
  arrowText(s,442,350,70,C.teal);
  const outs=[["MENU","菜单迭代"],["STAFFING","排班与介入"],["EQUIPMENT","维护与利用"],["CARE","持续照护"]];
  outs.forEach((d,i)=>{const x=530+(i%2)*340,y=215+Math.floor(i/2)*185; addShape(s,"roundRect",x,y,300,145,C.paper,{line:i===3?C.coral:C.teal,lineWidth:2,radius:"rounded-xl"}); addText(s,d[0],x+24,y+23,252,24,16,i===3?C.coral:C.teal,{bold:true}); addText(s,d[1],x+24,y+64,252,38,27,C.navy,{bold:true});});
  addText(s,"今天卖服务；长期积累站点 SOP、菜单模型、人员排班与设备维护经验。",180,590,920,34,22,C.teal,{bold:true,align:"center"});
  notes(s,"这页只讲运营学习，不宣称Data Moat。真实订单会产生用户、服务、厨房和社区四类数据，并反向改进菜单、排班、设备和照护。只有持续运营后，这些经验才可能形成壁垒。", []);
}
